...
def policy_summary():
    ...
    regional_ag_gpd = 14_000_000 * 325_851 / 365
    ...
    return {
        ...
        "pipeline_by_status": [
            {"status": r["status"], "count": r["count"], "total_mw": float(r["total_mw"]), "total_gpd": float(r["total_gpd"])}
            for r in pipeline
        ],
...
def industry_summary():
    ...
    renewable_mw = sum(float(r["total_mw"]) for r in ercot_by_fuel if r["fuel_type"] in renewable_fuels)
    ...
    return {
        ...
        "ercot_by_fuel": [
            {"fuel": r["fuel_type"], "mw": float(r["total_mw"]), "projects": r["project_count"], "pct": round(float(r["total_mw"]) / total_mw * 100, 1)}
            for r in ercot_by_fuel
        ],
        "tracked_sites": [
            {"id": s["id"], "name": s["name"], "operator": s["operator"], "county": s["county"], "status": s["status"],
             ...
             "lat": float(s["lat"]) if s["lat"] else None, "lon": float(s["lon"]) if s["lon"] else None}
            for s in sites
        ],
        "county_water_availability": [
            {"county": r["county"], "well_count": r["well_count"],
...
def public_summary():
    ...
    return {
        ...
        "by_county": [{"county": r["county"], "sites": r["cnt"], "mw": float(r["mw"]), "gpd": float(r["gpd"])} for r in by_county],
        "comparisons": {"households_equivalent": households_equivalent, "farm_acres_equivalent": farm_acres_equivalent, "olympic_pools_per_day": pools_per_day},
...
def compare_sites():
                ...
                SELECT s.id, s.name, s.project_code, s.operator, s.tenant,
                       ...
                       (SELECT AVG(wl.depth_to_water_ft)
                        ...
                        WHERE wns2.site_id = s.id
                          AND wl.measured_at >= now() - interval '2 years') AS nearby_avg_depth_ft
                FROM dc_sites s
    ...
    for s in sites:
        ...
        result.append({
            "id": s["id"], "name": s["name"], "project_code": s["project_code"],
            "operator": s["operator"], "county": s["county"], "status": s["status"],
            "capacity_mw": cap, "water_demand_gpd": gpd,
            "water_demand_acft_yr": round(gpd * 365 / 325_851, 0) if gpd else None,
            "water_intensity_gpd_per_mw": round(gpd / cap, 0) if (gpd and cap) else None,
